    ...
    for(i=loop_min;i<loop_max;i+=istep)
...
#if UNROLL
       if( i+3>DIM )
         printf("D[%d]: *** WARNING *** Unrolling loop creates a big problem! i=%d - need DIM divisable by 16\n",tid,i);
#endif
      for(j=0;j<DIM;j++)
       {  
#if UNROLL
	 ...
	 dot[0]=dot[1]=dot[2]=dot[3]=0.0;     
         for(k=0;k<DIM;k++) 
	  {
         ...
         c[i+3][j]=dot[3];
#else
         // Don't unroll the loop - let the compiler do this!
         dot0=0.0;      
         for(k=0;k<DIM;k++) 
           dot0 += a[i][k]*b[k][j];
         c[i][j]=dot0;
